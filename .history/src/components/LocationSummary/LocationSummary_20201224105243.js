...
const ForceSummary = ({ coords }) => {
  ...
  const [selectedSearchItem, setFilteredData] = useState([]);

  const onSearchItemSelect = (search) => {
    console.log('From the list!', search);
    setState({ selectedSearchItem: search });
    openModal();
  };  
        ...
        <div className="ui segment">
          <div className="ui row">
            <div className="six wide column">
              {data.length && <Searches searches={filteredData} onSearchItemSelect={onSearchItemSelect}
                      />/>}
            </div>
          </div>
        </div>
