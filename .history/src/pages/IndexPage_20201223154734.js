...
const IndexPage = () => {
  ...
  const [forces, setForces] = useData([])
  const [selectedForce, setSelectedForce] = useData(null);
  const [selectedForceName, setSelectedForceName] = useData('');
  const [selectedForceUrl, setSelectedForceUrl] = useData('');
  const [selectedForceDescription, setSselectedForceDescription] = useData('');
  ...
  const onSelectChange = (event) => {
    //console.log('Changed');
    const selectedForce = event.target.options[event.target.selectedIndex].value;
